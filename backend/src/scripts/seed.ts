...
import { UserRole, GameDifficulty } from '../types';
...
async function main() {
  ...
  try {
    ...
    const tutorPassword = await AuthService.hashPassword('tutor123');
    ...
    // Actualizar createdBy de la institución
    ...
    // Juego de Pares (el que mencionaste)
    ...
    for (const studentData of students) {
      ...
      // Crear progreso de ejemplo para algunos juegos
      if (Math.random() > 0.3) { // 70% probabilidad de tener progreso
